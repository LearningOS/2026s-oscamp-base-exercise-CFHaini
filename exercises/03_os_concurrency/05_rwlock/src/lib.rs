...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        // todo!()
        loop{
            let lock_state = self.state.load(Ordering::Acquire);

            if lock_state&WRITER_HOLDING!=0 || lock_state&WRITER_WAITING!=0{
                core::hint::spin_loop();
                continue;
            }
            if lock_state&READER_MASK==READER_MASK{
                core::hint::spin_loop();
                continue;
            }
            match self
                    .state
                    .compare_exchange(lock_state,lock_state+1,Ordering::AcqRel,Ordering::Acquire)
            {
                Ok(_)=>{
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop{
            let lock_state = self.state.load(Ordering::Acquire);
            if lock_state&READER_MASK!=0 || lock_state&WRITER_HOLDING!=0{
                core::hint::spin_loop();
                continue;
            }
            match self
